...
- **Chat commands** Guard forwards (and, once the mod queue feature ships, the
  song queue while a mod is viewing it) pass through the relay **in memory
  only**. Nothing is written to disk; operational logs record the channel and
  action, not message content.
